...
from std.read import (input_wrapper, read_single_int, read_until_pred,
                      read_until_pred_custom, read_single_float, read_list_of)

from lab5.domain.term_color import fore, style, InvalidColor
from lab5.domain.object_renderer import ALIGN_LEFT, ALIGN_CENTER, ALIGN_RIGHT
from lab5.domain.object_dict import get_object_dict
from lab5.domain.types import Point3F, Angle3F
from lab5.domain.geometry import get_3d_dist
...
def set_object_size_action(_: Config, state: AppState):
    ...
    print(f"Changed object width to {state.object_size[0] if state.object_size[0] > 0 else '[default object width]'}")
    print(f"Changed object height to {state.object_size[1] if state.object_size[1] > 0 else '[default object height]'}")
